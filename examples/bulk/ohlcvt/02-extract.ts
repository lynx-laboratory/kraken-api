/**
 * Bulk OHLCVT example: 02-extract
 *
 * What it does:
 * - Extracts OHLCVT ZIPs into the new extracted structure:
 *     .bulk-data/ohlcvt/extracted/complete/
 *     .bulk-data/ohlcvt/extracted/quarterly/<YYYYQn>/
 *
 * Run:
 *   yarn examples:bulk:ohlcvt:extract
 *
 * Modes (env vars):
 ...
 *         .bulk-data/ohlcvt/zips/quarterly/
 *
 *   QUARTER=2024Q4
...
async function main() {
  ...
  const extractOne = async (src: any, label: string) => {
    ...
    const res = await bulk.ohlcvt.extract(src, {
      onProgress,
      concurrency,
    });
  ...
  if (mode === 'complete') {
    await extractOne({ type: 'complete' }, 'Extracting OHLCVT complete');
    console.log('Next: yarn examples:bulk:ohlcvt:list-pairs');
    return;
  }
