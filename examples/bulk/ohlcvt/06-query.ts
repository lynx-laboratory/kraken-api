/**
 * Bulk OHLCVT example: 06-query
 *
 * What it does:
 * - Streams OHLCVT rows from an extracted CSV for a given pair + interval
 ...
 *
 * Run:
 *   yarn examples:bulk:ohlcvt:query
 *
 * Env vars:
 *   PAIR=XBTUSD        Pair symbol (CSV filename prefix)
 *   INTERVAL=60        Candle interval (1,5,15,30,60,240,720,1440)
...
async function main() {
  ...
  const interval = Number(process.env.INTERVAL ?? '60'); // 1,5,15,30,60,240,720,1440
  ...
  let n = 0;
  for await (const row of bulk.ohlcvt.query(
    { pair, interval: interval as any, source, startTs, endTs } as any,
    { limit },
  )) {
    console.log(row);
    n += 1;
