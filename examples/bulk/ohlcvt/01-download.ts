/**
 * Bulk OHLCVT example: 01-download
 *
 * What it does:
 * - Downloads OHLCVT ZIPs into the new zip structure:
 *     .bulk-data/ohlcvt/zips/complete/complete.zip
 *     .bulk-data/ohlcvt/zips/quarterly/<YYYYQn>.zip
 *
 * Run:
 *   yarn examples:bulk:ohlcvt:download
 *
 * Modes (env vars):
 ...
 *
 * Notes:
 * - all-quarters requires bulk.ohlcvt.listAvailableQuarters() to exist.
...
async function main() {
  ...
  const downloadOne = async (src: any, label: string) => {
    ...
    const res = await bulk.ohlcvt.download(
      src,
  ...
  if (mode === 'complete') {
    await downloadOne({ type: 'complete' }, 'Downloading OHLCVT complete');
    return;
  }
  ...
  if (mode === 'quarter') {
    await downloadOne(
      ...
      `Downloading OHLCVT ${quarter}`,
    );
    return;
  }

  // mode === 'all-quarters'
  const quarters = await (bulk.ohlcvt as any).listAvailableQuarters?.();
  if (!Array.isArray(quarters) || quarters.length === 0) {
    throw new Error(
      'bulk.ohlcvt.listAvailableQuarters() is missing or returned no quarters. ' +
        'Add the public method to the OHLCVT API to enable MODE=all-quarters.',
    );
  }
